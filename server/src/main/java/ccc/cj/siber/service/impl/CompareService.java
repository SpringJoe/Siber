...
public class CompareService implements ICompareService {
    private static final Logger logger = LogManager.getLogger(CompareService.class);
...
//        tables.parallelStream().forEach(table -> {
        tables.stream()
                ...
                .forEach((Table table) -> {
                    logger.debug("begin compare, table name is " + table.getTableName());
...
//                                logger.debug("compareStr is " + compareStr);
                    ...
                    logger.debug("finish compare, table name is " + table.getTableName());
                });
        Result<List<TableCompare>> result = new Result<>(tableCompares);
        logger.debug("compare result is " + new ObjectMapper().writeValueAsString(result));
        return result;
    }
